...
```{r}
image_path <- image_selection$image_path[1]
blob_filename <- image_selection$blob_filename[1]
```
...
- create_grey_value_image

```{r}
blob_parameter <- read_XML_BLOB_parameter(selection_group$image_path[1],
                                  selection_group$blob_filename[1])
  
  blob_image <- data_parameter%>%
    read_image_binary_file()
```
...
```{r}
image_selection <- select_valid_image_files(image_files,
                                         type = "blob32")
```
...
```{r}
image_selection_list <- image_selection%>%
  dplyr::group_by(chip_ID,pos_ID)%>%
  tidyr::nest()
```

- import all images of a single selection group

```{r}
selection_group <- image_selection_list$data[[1]]
```

- map over all entities and load image data into a list 
--> dauert ewig

```{r}
group_data <- purrr::map2(selection_group$image_path,
                          selection_group$blob_filename,
                          ~ read_XML_BLOB_parameter(.x,.y)%>%
                            read_image_binary_file)
```

- calculate data_sum of a chip_group directly and return only one dataset per image selection group

```{r}

for(i in 1:dim(selection_group)[1]){
  
  blob_parameter <- read_XML_BLOB_parameter(selection_group$image_path[i],
                                  selection_group$blob_filename[i])
