---
title: " documentation - find_all_BlobImageFiles"
...
knit: (function(input, ...) {
    rmarkdown::render(
      input,
      output_file = "inst/report/dev report - find all Blob image files.html",
      envir = globalenv()
    )
  })
---
...
IDs <- IDs%>%
  dplyr::mutate(result = query_mongoDB("scans",
                                       "channelUID",
                                       chip_IDs)$result)

# select columns
IDs <- IDs%>%
  dplyr::mutate(result =purrr::map(result,
                  ~.x%>%
...
IDs <- IDs%>%
  dplyr::mutate(result = purrr::map(result,
                                    ~.x%>%
                                      ...
                                      dplyr::select(chip_ID, scan_ID, "pos_ID" = "posid", jobType, basePath, Status, chipx, chipy, enabled,`enabled-count`)))

IDs <- IDs%>%dplyr::select(-chip_ID)%>%
...
```{r}
get_image_folders <- function(imageServer_path,basePath,pos_ID){
  
  path <- file.path(imageServer_path,basePath,
                    paste0("pos",pos_ID))
...
- columns unnested position column:
 [1] "chip_ID"           "scan_ID"           "jobType"           "chip_path"         "Status"            "posid"            
 [7] "focusOffset"       "chipx"             "chipy"             "stageX"            "stageY"            "stageZ"           
[13] "imagePixDX"        "imagePixDY"        "imageRotation"     "posref"            "hdr"               "flimages"         
[19] "flimage-count"     "bleach-time"       "enabled"           "focusInterpolated" "focus"             "deltaTL"          
[25] "deltaTL-count"     "enabled-count" 

```{r}
IDs <- IDs%>%
  dplyr::mutate(pos_path = file.path(imageServer_path,basePath,
                    paste0("pos",pos_ID)))
...
IDs <- IDs%>% dplyr::group_by(chip_ID)%>%tidyr::nest()%>%dplyr::mutate(ScanHistory = purrr::map(chip_ID,~create_ScanHistory_of_chipIDs(.x)))

IDs <- IDs%>%tidyr::unnest()
```
...
**query chip_ID in collection scans results in**

- a dataframe without EDL field filled
- important containing columns:

    + basepath= chip_path
    + jobType either BLEACH, TL or FL 
    + UID = scan_ID
    + channelUID = chip_ID

- good to know columns imageRotation, positionCount, hardware, positions
...
- good to know columns: 
    ...
    + positions, df containing columns: "posid", "focusOffset", "enabled", "focusInterpolated" "chipx", "chipy", "stageX", "stageY", "stageZ", "imagePixDX", "imagePixDY", "imageRotation", "posref", "focus", "deltaTL", "deltaTL-count"  
...
# query for all scan_IDs in scans-collection
scan_IDs <- purrr::map(ScanHistory,~.x$scan_ID)%>%purrr::flatten_chr()
query_result <- query_UID_scans(scan_IDs)
result <- get_df_from_query_result(query_result)

# get image type of each scan
image_Type <- dplyr::tibble(
  scan_ID = result$UID,
  image_Type = result$jobType,
...
#IDs <- IDs%>%
#  dplyr::mutate(
#    BlobImage_filepath = dplyr::case_when(image_Type == )
#  )

# add HDRimage_filepath
IDs <- IDs%>%
  dplyr::mutate(
    HDRimage_filepath = create_Pos_image_filepath(ScanBasePath,PositionFolder,"hdr"))
# add FLimage_filepath
IDs <- IDs%>%
  dplyr::mutate(
    FLimage_filepath = create_Pos_image_filepath(ScanBasePath,PositionFolder,"flimages"))
# add posRefimage_filepath
IDs <- IDs%>%
  dplyr::mutate(
    posRefimage_filepath = create_Pos_image_filepath(ScanBasePath,PositionFolder,"posRef"))
# add deltaTL_filepath
IDs <- IDs%>%
  dplyr::mutate(
    deltaTLimage_filepath = create_Pos_image_filepath(ScanBasePath,PositionFolder,"deltaTL"))
# add focus_filepath
IDs <- IDs%>%
  dplyr::mutate(
    focusimage_filepath = create_Pos_image_filepath(ScanBasePath,PositionFolder,"focus"))
...
MH <- MethodHistory%>%
  ...
  dplyr::filter(UID %in% missing_scan_IDs)

result%>%
  dplyr::filter(UID %in% missing_scan_IDs)%>%
  dplyr::pull(channelUID)%>%
  table()
